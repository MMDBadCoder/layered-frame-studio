...
class BrandingTests(StudioTestCase):
    """The brand mark is regenerated by scripts/make_logo.py, so these assert
    the wiring rather than any particular geometry."""

    @staticmethod
    def logo_shapes():
        """The <rect> lines of the current mark, read from the partial."""
        from django.conf import settings

        partial = Path(settings.BASE_DIR) / "templates/partials/logo.html"
        return [line.strip() for line in partial.read_text(encoding="utf-8").splitlines()
    ...
    def test_pages_carry_the_logo_and_a_favicon_set(self):
        ...
        shapes = self.logo_shapes()
        self.assertGreater(len(shapes), 0, "the logo partial has no shapes")
        for shape in shapes:
            self.assertIn(shape, html, "the header is not showing the current mark")
    ...
    def test_the_orders_page_uses_the_same_mark(self):
        ...
        for shape in self.logo_shapes():
            self.assertIn(shape, html)

    def test_the_generated_assets_all_exist_and_are_transparent(self):
        from django.conf import settings

        images = Path(settings.BASE_DIR) / "static/img"
        for name in ("logo.png", "icon-16.png", "icon-32.png"):
            with Image.open(images / name) as icon:
                self.assertEqual(icon.mode, "RGBA", f"{name} has no alpha channel")
                self.assertEqual(icon.getpixel((0, 0))[3], 0, f"{name} has an opaque corner")

        # The touch icon is deliberately opaque: iOS ignores transparency.
        with Image.open(images / "icon-180.png") as touch:
            self.assertEqual(touch.mode, "RGB")
